...
import ca.phon.syllabifier.io.SonorityValues;

/**
 * @author Greg
 *
 */
public class SonorityScale extends SonorityValues {
	 ...
	 */
	public int calculateSonorityDistance(Phone comp1, Phone comp2) {
	...
	private int getSonorityValue(Phone phone) {
		// rules are checked in the order they are specified in
		// the xml description.  The value from the first matched
		// rule is used
		for(SonorityClass sonorityDef:getSonorityClass()) {
			Constraint currentConstraint = 
				(Constraint)sonorityDef.getConstraint();
