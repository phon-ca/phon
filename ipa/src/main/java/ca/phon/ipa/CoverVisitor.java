...
//				}
    ...
    public void visitCompoundPhone(CompoundPhone cp) {
        ...
        final CompoundPhone newCp = factory.createCompoundPhone(c1, c2, cp.getLigature(), cp.overrideFeatureSet(), cp.syllableInfo());
    ...
    private Phone cover(Phone p) {
        ...
        final Phone newPhone =
                includeDiacritics ? factory.createPhone(p.getPrefixDiacritics(), baseChar, p.getCombiningDiacritics(), p.getSuffixDiacritics(), p.overrideFeatureSet(), p.syllableInfo())
                        : factory.createPhone(new Diacritic[0], baseChar, new Diacritic[0], new Diacritic[0], p.overrideFeatureSet(), p.syllableInfo());
        return newPhone;
    }
