...
/*
 * Modified for use in Phon Nov 2009 by Greg Hedlund
 * 
 * -GJH1- FeatureSet literals
 * Added rules for feature set literals used in Phon queries.
 * i.e., ^{ "Consonant", "Vowel", ... }^
 */
...
// GJH 18 Nov 2009
// Tokens for feature set literals
	FSSTART		= '^{';
	FSEND		= '}^';
...
// GJH 18 Nov 2009
// Added FEATURESET imaginary token
	FEATURESET ;
	FSELE ;
}
...
featureSetItem
	: ( fsele=featureSetElement | { input.LA(1) == COMMA }? )
	-> ^(FSELE $fsele?)
	;
	
featureSetLiteral
	: fsstart=FSSTART ( featureSetItem ( COMMA featureSetItem )* )? FSEND 
	-> ^( FEATURESET[$fsstart, "FEATURESET"] featureSetItem* )
	;
