...
public class IPAToUserTierAligner implements ITierAligner<IPATranscript, IPATranscript, UserTierData, UserTierElement> {
    ...
    @Override
    public TierAlignment<IPATranscript, IPATranscript, UserTierData, UserTierElement>
        calculateAlignment(Tier<IPATranscript> topTier, Tier<UserTierData> bottomTier, TierAlignmentRules alignmentRules) {
        final List<IPATranscript> ipaWords = topTier.getValue().words().stream().filter(new IPAAlignmentFilter(alignmentRules)).toList();
