...
public class BreakableFlowLayout implements LayoutManager, Serializable {
     ...
     */
    private boolean alignOnBaseline = true;

    /**
     * Use serialVersionUID from JDK 1.1 for interoperability.
     */
    @Serial
    private static final long serialVersionUID = -8221234835583281478L;

    /**
     * Constructs a new {@code FlowLayout} with a centered alignment and a
     * default 5-unit horizontal and vertical gap.
     */
    public BreakableFlowLayout() {
        this(LEADING, 5, 5);
    }
     ...
     */
    public BreakableFlowLayout(int align) {
        this(align, 5, 5);
    }
     ...
     *                     borders of the {@code Container}
     */
    public BreakableFlowLayout(int align, int hgap, int vgap) {
        this.hgap = hgap;
        this.vgap = vgap;
        setAlignment(align);
    }
    ...
    public Dimension preferredLayoutSize(Container target) {
        synchronized (target.getTreeLock()) {
            ...
            int maxWidth = (getBreakWidth() > 0 ? getBreakWidth() : target.getWidth()) - (insets.left + insets.right);
            int maxHeight = 0;
            ...
            if (useBaseline) {
                maxHeight = Math.max(maxAscent + maxDescent, maxHeight);
            }

            final int prefWidth = (numRows > 1 ? maxWidth : x) + insets.right;
            final int prefHeight = y + maxHeight + insets.bottom;
    ...
    public void layoutContainer(Container target) {
        synchronized (target.getTreeLock()) {
            Insets insets = target.getInsets();
            int maxwidth = (getBreakWidth() > 0 ? getBreakWidth() : target.getWidth())
