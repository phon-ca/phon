...
function query_record(recordIndex, record) {
	...
	for (var gIdx = 0; gIdx < searchObjects.length; gIdx++) {
		...
		for (var wIdx = 0; wIdx < words.length; wIdx++) {
			...
			if ((pccOptions.standard.includePcc == true || pccOptions.aligned.includePcc == true) && (pccOptions.standard.includePvc == false && pccOptions.aligned.includePvc == false) && ipaT.indexOf("\\c") < 0) {
				continue;
			}
			if ((pccOptions.standard.includePcc == false && pccOptions.aligned.includePcc == false) && (pccOptions.standard.includePvc == true || pccOptions.aligned.includePvc == true) && ipaT.indexOf("\\v") < 0) {
				continue;
			}
